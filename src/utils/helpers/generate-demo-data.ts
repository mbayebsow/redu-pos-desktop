...
function generateProducts(numItems: number, categories: CategoryType[], suppliers: SupplierType[]) {
  ...
  for (let i = 0; i < numItems; i++) {
    ...
    const image = "https://dummyimage.com/100x100/e2e2e2/000000.png";
    const supplier = suppliers[Math.floor(Math.random() * suppliers.length)].id;
...
function generateProductOptions(numItems: number, ProductID: number, identifier: string, supplier: number | null) {
...
function generateData() {
  ...
  productsdb
    ...
    .map((product) => {
      ...
      const productOptions = generateProductOptions(randomNumber, product.id, product.identifier, product.supplier);
