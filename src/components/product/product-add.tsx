...
function ProductAdd() {
  ...
  return (
    <div className="gap-3 flex flex-col bg-primary-50 border border-primary-light p-3 rounded-lg">
      ...
      <TextField label="Nom du produit" name="name" type="text" placeholder="Sucre" value={newProductState.name} onChange={handleNewProductValue} />
      <div className="flex flex-row gap-2 mb-2">
        ...
        <SelectField
          ...
          placeholder="Choisir"
          name="unite"
          value={newProductState.unite}
          options={["Unité", "Cartons", "Paquets", "Packs", "Bouteilles", "Sacs", "Kg"]}
          onChange={handleNewProductValue}
        />
      </div>
      <SwitchField label="Prix moitié" name="priceDemi" checked={newProductState.priceDemi} onChange={handleNewProductValue} />
    </div>
  );
}
